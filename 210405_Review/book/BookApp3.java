...
public class BookApp3 {
	public static void main(String[] args) {
		...
		//event2 객체 생성
		//gift1 객체 생성
		Event event1 = new Event("20%할인","4월 구매도서 할인행사","4/1","4/31",false);
		Event event2 = new Event("신규가입 할인","4월 두번째 가입고객 할인","4/5","4/12",false);
		Gift gift1 = new Gift("머그컵", 3000);

		//event1과 gift1이 각각 참조하는 Event 객체와 Gift객체의 주소값을 생성자의 매개변수로 전달
		Book book1 = new Book(100,"이것이 자바다","신용권","한빛미디어",38000,false,0.15,event1,gift1);
		//event2가 참조하는 Event객체의 주소값과, 객체를 직접 생성해서 그 객체의 주소값이 생성자의 매개변수로 전달
		Book book2 = new Book(120,"자바의 정석","남궁성","도우출판",25000,false,0.15,event2,gift1);
		...
		book3.setEvent(new Event("특가할인","추가구매 고객대상", "4/1","4/14",false));
